<template>
	<div>
		<div class="movie-genre-container p-4 pb-2">
			<div
				v-for="(item, index) in compDataTv"
				:key="`list populer movie - ${index}`"
				class="movie-genre-list"
			>
				<button class="poster-movie" @click="detailTv(item)">
					<div
...
export default {
	...
	async setup() {
		...
		const loadMoreMovie = async () => {
			currPage.value += 1;
			const res = await api().tv.getListPopulerTv({
				language: "en-US",
				page: currPage.value,
			});
			res.data.results.map((item) => {
				dataTv.value.push(item);
			});
		};
		...
		const getTvPopuler = await api().tv.getListPopulerTv({
			language: "en-US",
			page: currPage.value,
		});

		if (dataTv.value === null) {
			dataTv.value = getTvPopuler.data.results;
		}
